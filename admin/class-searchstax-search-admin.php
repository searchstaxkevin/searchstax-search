...
class Searchstax_Search_Admin {
	...
	public function add_options_page() {
		...
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_suggest_url', array( $this, 'sanitize_token' ) );
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_related_token', array( $this, 'sanitize_token' ) );
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_related_url', array( $this, 'sanitize_token' ) );
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_smart_url', array( $this, 'sanitize_token' ) );
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_analytics_token', array( $this, 'sanitize_token' ) );
		register_setting( 'searchstax_search_account', 'searchstax_search_searchui_app_id', array( $this, 'sanitize_token' ) );
	}
	...
	public function update_schema() {
		...
		if ( $api['available'] ) {
			...
			$args = array(
				'body' => '{
					"add-field": { 
						"name": "site", "type": "string", "indexed": true, "required": false, "stored": true, "multiValued": false
					},
					"add-field": { 
						"name": "title", "type": "string", "indexed": true, "required": false, "stored": true, "multiValued": true
					},
					"add-field": {
						"name": "url", "type": "string", "indexed": true, "required": false, "stored": true, "multiValued": false
					},
					"add-field": {
						"name": "body", "type": "text", "indexed": true, "required": false, "stored": true, "multiValued": true
					},
					"add-field": {
						"name": "summary", "type": "text", "indexed": true, "required": false, "stored": true, "multiValued": true
					},
					"add-field": {
	...
	public function post_to_solr_json( $post, $solr_id ) {
		...
		$solrDoc['id'] = $solr_id;
		$solrDoc['site'] = get_bloginfo('name');
	...
	public function edit_search_result() {
		...
		if (isset($_POST['search_status']) && $_POST['search_status'] == 'publish' ) {
			if ( isset($_POST['search_page_id']) && get_post_status($_POST['search_page_id']) == 'publish') {
				...
				update_post_meta($post_id, 'search_profile', $_POST['search_profile']);
				...
				add_post_meta($post_id, 'search_profile', $_POST['search_profile'], true);
