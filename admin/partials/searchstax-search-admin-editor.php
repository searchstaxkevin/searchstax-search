	...
	<div>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<div>
				<h2>Result Config</h2>
				<div>
					<input type="radio" value="config_static" name="search_config" <?php if ( (isset($meta['search_config']) && $meta['search_config'][0] == "config_static") || !isset($meta['search_config']) ) { echo 'checked'; } ?> class="searchstax_search_admin_resultconfig" />
					<label for="config_static">Static</label>
				</div>
				<div>
					<input type="radio" value="config_dynamic" name="search_config"  <?php if (isset($meta['search_config']) && $meta['search_config'][0] == "config_dynamic") { echo 'checked'; } ?> class="searchstax_search_admin_resultconfig" />
					<label for="config_dynamic">AJAX/Dynamic</label>
				</div>
				<div>
					<input type="radio" value="config_searchui" name="search_config"  <?php if (isset($meta['search_config']) && $meta['search_config'][0] == "config_searchui") { echo 'checked'; } ?> class="searchstax_search_admin_resultconfig" id="searchstax_search_admin_searchui" />
					<label for="config_searchui">Search UI Kit</label>
					<p>Search UI Kit display options can be configured in the SearchStax Site Search dashboard</p>
				</div>
			</div>
			<div>
				<h2>Search Profile</h2>
				<div>
					<input type="text" name="search_profile" value="<?php 
					if ( isset($meta['search_profile']) ) {
						echo $meta['search_profile'][0];
					}
					?>" />
					<p>Use a specific search profile for this search page
				</div>
			</div>
			<div>
				<h2>Display Results</h2>
				<div>
					<input type="radio" value="display_inline" name="search_display" <?php if ( (isset($meta['search_display']) && $meta['search_display'][0] == "display_inline") || !isset($meta['search_display']) ) { echo 'checked'; } ?> class="searchstax_search_admin_searchui_disable" />
					<label for="display_inline">Inline</label>
				</div>
				<div>
					<input type="radio" value="display_grid" name="search_display"  <?php if ( isset($meta['search_display']) && $meta['search_display'][0] == "display_grid") { echo 'checked'; } ?> class="searchstax_search_admin_searchui_disable" />
			...
			<div>
				<h2>Search Bar</h2>
				<div>
					<input type="radio" value="user_search" name="search_bar" <?php if ( (isset($meta['search_bar']) && $meta['search_bar'][0] == "user_search") || !isset($meta['search_bar']) ) { echo 'checked'; } ?> class="searchstax_search_admin_searchui_disable" />
					<label for="user_search">User Search Query (Show Search Bar)</label>
				</div>
				<div>
					<input type="radio" value="fixed_search" name="search_bar"  <?php if ( isset($meta['search_bar']) && $meta['search_bar'][0] == "fixed_search") { echo 'checked'; } ?> class="searchstax_search_admin_searchui_disable" />
				...
				<div>
					...
					?>" class="searchstax_search_admin_searchui_disable" />
					<p>Use <code>*</code> to show all results</p>
				</div>
			</div>
			<div>
				<h2>Result Count</h2>
				<select name="search_result_count" class="searchstax_search_admin_searchui_disable">
				...
				<?php
					foreach ( $post_types as $index => $this_post ) {
						if ( !in_array($this_post, $unindexable_types) ) {
							...
							echo ' class="searchstax_search_admin_searchui_disable"/><label for="' . $this_post . '">' . ucfirst($this_post) . '</label>';
							echo '</div>';
						}
					}
				?>
			...
			<div>
				...
				<?php
					foreach ( $categories as $index => $this_category ) {
						...
						echo ' class="searchstax_search_admin_searchui_disable"/><label for="' . $this_category->name . '">' . $this_category->name . '</label>';
						echo '</div>';
					}
				?>
				</div>
			...
			<div>
				...
				<?php
					foreach ( $tags as $index => $this_tag ) {
						...
						echo ' class="searchstax_search_admin_searchui_disable"/><label for="' . $this_tag->name . '">' . $this_tag->name . '</label>';
						echo '</div>';
					}
				?>
				</div>
